class ArticlesController < ApplicationController
  before_filter :check_administarator_role, :except => [:index, :show]
  def index
    if params[:category_id]
      @articles_pages = Article.paginate(
        :page => params[:page],
  ...
  def admin
  	@articles_pages = Article.paginate(:page => params[:page], 
  									   :order => 'published_at DESC' )
  									   
  	#@test = Article.paginate :page => params[:page], :order => 'published_at DESC'
  end
